...
void Material::bind() const
{
    ...
    _program->set_uniform("BaseColor", _BaseColor);
    _program->bind();
}
...
Material Material::colored_material()
{
    Material material;
    material._program = Program::from_files("lit.frag", "instanced.vert");
    return material;
}
...
Material Material::textured_transparent_material()
{
    Material material;
    material._program = Program::from_files(
        "lit.frag", "instanced.vert",
        std::array<std::string, 2>{ "TEXTURED", "TRANSPARENT" });
    return material;
}
...
Material Material::textured_normal_mapped_transparent_material()
{
    Material material;
    material._program =
        Program::from_files("lit.frag", "instanced.vert",
                            std::array<std::string, 3>{
                                "TEXTURED", "NORMAL_MAPPED", "TRANSPARENT" });
...
Material Material::textured_masked_material(float alpha_cutoff)
{
    ...
    material._program =
        Program::from_files("g_buffer.frag", "instanced.vert",
                            std::array<std::string, 2>{ "TEXTURED", "MASKED" });
...
Material Material::textured_normal_mapped_masked_material(float alpha_cutoff)
{
    Material material;
    material._program = Program::from_files(
        "g_buffer.frag", "instanced.vert",
        std::array<std::string, 3>{ "TEXTURED", "NORMAL_MAPPED", "MASKED" });
    material.alpha_cutoff = alpha_cutoff;
    return material;
}
