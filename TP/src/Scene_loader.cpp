...
Result<std::unique_ptr<Scene>> Scene::from_gltf(const std::string &file_name)
{
    ...
    for (auto [node_index, node_transform] : node_transforms)
    {
        ...
        for (size_t j = 0; j != mesh.primitives.size(); ++j)
        {
            ...
            if (prim.material >= 0)
            {
                std::vector<double> base_color = { 1.0, 1.0, 1.0, 1.0 };
                ...
                if (!mat)
                {
                    ...
                    base_color = gltf.materials[prim.material]
                                     .pbrMetallicRoughness.baseColorFactor;
                    const auto &normal_info =
                        gltf.materials[prim.material].normalTexture;
                    const auto &alpha_info =
                        gltf.materials[prim.material].alphaMode;
                    const auto &alpha_cutoff =
                        gltf.materials[prim.material].alphaCutoff;
                    std::cout << "alpha_info: " << alpha_info
                              << " for material name: "
                              << gltf.materials[prim.material].name
                              << "with index: " << prim.material << std::endl;
                    std::cout << "alpha_cutoff: " << alpha_cutoff
                              << " for material name: "
                              << gltf.materials[prim.material].name
                              << "with index: " << prim.material << std::endl;
                    std::cout << "base_color: " << base_color[0] << " "
                              << base_color[1] << " " << base_color[2] << " "
                              << base_color[3] << " for material name: "
                              << gltf.materials[prim.material].name
                              << "with index: " << prim.material << std::endl;
                    auto load_texture =
                        [&](auto texture_info,
                        ...
                        {
                            std::cerr
                                << "Unsupported texture coordinate channel ("
                    ...
                    if (!albedo)
                    {
                        if (base_color[0] != 1.0 || base_color[1] != 1.0
                            || base_color[2] != 1.0 || base_color[3] != 1.0)
                        {
                            mat = std::make_shared<Material>(
                                Material::colored_material());
                        }
                        else
                        {
                    ...
                    else if (!normal)
                    {
                        if (alpha_info == "BLEND")
                            mat = std::make_shared<Material>(
                                Material::textured_transparent_material());
                        else if (alpha_info == "MASK")
                            mat = std::make_shared<Material>(
                                Material::textured_masked_material(
                                    alpha_cutoff));
                        else
                            mat = std::make_shared<Material>(
                    ...
                    else
                    {
                        if (alpha_info == "BLEND")
                            mat = std::make_shared<Material>(
                                Material::
                                    textured_normal_mapped_transparent_material());
                        else if (alpha_info == "MASK")
                            mat = std::make_shared<Material>(
                                Material::
                                    textured_normal_mapped_masked_material(
                                        alpha_cutoff));
                        else
                            mat = std::make_shared<Material>(
                ...
                glm::vec4 base_color_vec = glm::vec4(
                    base_color[0], base_color[1], base_color[2], base_color[3]);
                mat->set_base_color(base_color_vec);

                material = mat;
            }

            auto scene_object = SceneObject(
                std::make_shared<StaticMesh>(mesh.value), std::move(material));
    ...
    std::cout << "    " << scene->point_lights().size() << " point lights"
              << std::endl;
    std::cout << "    " << textures.size() << " textures" << std::endl;
    for (auto [id, texture] : textures)
    {
        std::cout << "        " << id << ": " << texture->size().x << "x"
                  << texture->size().y << std::endl;
    }
